...
struct GLTF {
	...
	images.resize(asset.images.size(), {ctx, VK_NULL_HANDLE});
	for(size_t i = 0; i < asset.images.size(); i++) {
		...
		vb::log(std::format("Loading {}...", path.c_str()));
    		...
    		images[i].image.create(path.c_str(), true);
		...
		assert(data);
    		...
    		images[i].image.create(data, size, true);
    		assert(images[i].image.all_valid());
	...
	std::vector<VkDescriptorPoolSize> sizes = {
	    {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (uint32_t)images.size()}
	};
	...
	for(auto& image: images) {
	    image.descriptor = descriptor.create_set(descriptor_layout);
	    assert(image.descriptor);
	    VkDescriptorImageInfo info {
		.sampler = sampler,
		.imageView = image.image.image_view,
	...
	VkSamplerCreateInfo sampler_info = {
	    ...
	    .minLod = 0.0f,
	    .maxLod = (float)images[0].image.mip_level,
